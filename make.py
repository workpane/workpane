...
# Every catalog of the product is one plugin header plus the shell, and naming them once is what keeps four audits reading the same set.
def translation_catalogs() -> list[Path]:
    return sorted(ROOT.glob("plugins/*/*Translations.h")) + [ROOT / "src" / "plugins" / "CoreTranslations.h"]
...
# The call is held to the most a sentence declares, so a key that declares one argument in a language and none in another is invisible there.
# Qt drops the argument in whichever language forgot it, and the reader of that language meets a sentence missing its number.
def divergent_translation_arguments() -> list[str]:
    entry = re.compile(r"\{QStringLiteral\(\"([a-z0-9.\-]+)\"\), QStringLiteral\(\"((?:[^\"\\\\]|\\\\.)*)\"\)\}")
    counted: dict[str, dict[str, int]] = {}

    for path in translation_catalogs():
        for language, body in re.findall(r"static TranslationEntries (\w+)\(\)\s*\{(.*?)\n    \}", path.read_text(encoding="utf-8"), re.S):
            for key, value in entry.findall(body):
                marks = [int(mark) for mark in re.findall(r"%(\d)", value)]
                counted.setdefault(key, {})[f"{path.name}:{language}"] = max(marks) if marks else 0

    found = []

    for key, languages in sorted(counted.items()):
        if len(set(languages.values())) > 1:
            spelled = ", ".join(f"{where} declares {count}" for where, count in sorted(languages.items()))
            found.append(f"{key} is given a different number of arguments by its own languages, because {spelled}")

    return found


# A notification carries a sentence of the catalog of whoever shows it, because the message of a structured error is written for the log and in one language.
# The shape a helper hides behind a parameter is not read here, so the one that hands a diagnostic straight to a notification is what this refuses.
def notifications_carrying_diagnostics() -> list[str]:
    shown = re.compile(r"notify\([^;]{0,400}?(?:\.error\(\)|\berror)\.(?:message|detail)\b")
    handed = re.compile(r"\(\s*[\w.>\-]*\.?error\(\)\s*,\s*[^;()]{0,120}?\.error\(\)\.(?:message|detail)\b")
    ...
    for directory in ("src", "plugins"):
        for path in sorted((ROOT / directory).rglob("*.cpp")):
            for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
                # A key of the catalog spells the words this looks for, so the text of every literal is taken out before it matches.
                line = literal.sub('""', line)

                if shown.search(line) or handed.search(line):
                    found.append(f"{path.relative_to(ROOT)}:{number} shows the reader a message written for the log")
...
# An operator left at the end of a line is a statement the formatter never wrote, so every one that really continues is named here.
# The increment, the scope resolution and the label of an access specifier are left alone, because none of them continues anything.
    ...
    for directory in ("src", "plugins", "tests"):
        for path in sorted((ROOT / directory).rglob("*")):
            ...
            for index, line in enumerate(lines[:-1]):
                ...
                if not CONTINUED_STATEMENT.search(stripped) or not following or following.startswith("}"):
                    continue
...
# A member is written once where it is declared, so one the product really reaches carries its name somewhere else as well.
# Two classes may spell one member name, and a name reached from either of them answers for both, so this refuses nothing it is unsure of.
def unused_members() -> list[str]:
    # The type may carry a signature of its own, which is what a member holding a callable is written as.
    declaration = re.compile(r"^\s+(?:mutable\s+)?[A-Za-z_][\w:<>,\s\*&()]*\s(m_\w+)\s*(?:\{[^}]*\}|=[^;]*)?;")
        ...
        for path in sorted((ROOT / directory).rglob("*.h")):
            for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
                ...
                if match is not None:
                    declared.setdefault(match.group(1), f"{path.relative_to(ROOT)}:{number}")
    ...
    for name, where in sorted(declared.items()):
        if len(re.findall(r"\b" + re.escape(name) + r"\b", sources)) <= 1:
            found.append(f"the member {name} in {where} is read by nothing")
    ...
    for name in source_files():
        ...
        for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            if line.startswith(" ") or line.startswith("\t") or stripped.startswith("//"):
                continue
            # A definition is what a source carries, so a variable built from a constructor call is not one of these.
            if path.suffix == ".cpp" and not stripped.endswith("{"):
                continue
            ...
            if match is not None and not qualified and match.group(1) not in ("if", "while", "for", "return", "switch", "main"):
...
def narrowly_computed_bounds() -> list[str]:
    found: list[str] = []
    declaration = re.compile(r"constexpr\s+(qsizetype|qint64|quint64|std::size_t|std::uint64_t|std::int64_t)\s+(\w+)\s*=\s*([^;]+);")
    ...
    for directory in ("src", "plugins"):
        for path in sorted((ROOT / directory).rglob("*")):
            ...
            for number, line in enumerate(path.read_text(encoding="utf-8").split("\n"), 1):
                ...
                first = match.group(3).split("*")[0].strip()

                if re.fullmatch(r"\d+", first):
                    found.append(f"{path.relative_to(ROOT)}:{number} computes {match.group(2)} in int and widens it to {match.group(1)}")

    return found


def unmatchable_header_filter() -> list[str]:
    configuration = ROOT / ".clang-tidy"

    if not configuration.exists():
        return ["the static analysis configuration is missing"]

    for number, line in enumerate(configuration.read_text(encoding="utf-8").split("\n"), 1):
        if not line.startswith("HeaderFilterRegex:"):
            continue

        expression = line.split(":", 1)[1].strip().strip("'\"")

        if re.search(expression, "/workpane/src/ui/Theme.h") is None:
            return [f".clang-tidy:{number} declares a header filter that matches no absolute path, so no header is analysed"]

        if re.search(expression, "/workpane/build/debug/_deps/hwinfo-src/include/hwinfo/cpu.h") is not None:
            return [f".clang-tidy:{number} declares a header filter that reaches a pinned dependency"]

        return []

    return ["the static analysis configuration declares no header filter"]


def task_audit(_: Context) -> None:
    narrow = narrowly_computed_bounds()

    if narrow:
        raise RuntimeError("A bound is computed in the width it is declared as, because the product it is written as must never be the narrower one:\n  " + "\n  ".join(narrow))

    unmatchable = unmatchable_header_filter()

    if unmatchable:
        raise RuntimeError("Static analysis reads the headers of this product, so its filter matches the absolute path it is given and reaches nothing pinned:\n  " + "\n  ".join(unmatchable))
    ...
    diagnostic = notifications_carrying_diagnostics()

    if diagnostic:
        raise RuntimeError("A notification carries a sentence of the catalog of whoever shows it, because the message of a structured error is written for the log and in one language:\n  " + "\n  ".join(diagnostic))

    divergent = divergent_translation_arguments()

    if divergent:
        raise RuntimeError("Every language of a key declares the same arguments, because the one that forgot drops it and leaves the reader a sentence missing its number:\n  " + "\n  ".join(divergent))
    ...
    orphaned = unused_members()

    if orphaned:
        raise RuntimeError("A member nothing reads is one nobody removed, so every member declared is reached from somewhere:\n  " + "\n  ".join(orphaned))
...
def validate_macos_bundle(bundle: Path) -> None:
    ...
    # A package that carries no driver opens no database, and one that carries no backend reaches no provider, so neither is left to the first reader who downloads it.
    for plugin in ("platforms/libqcocoa.dylib", "sqldrivers/libqsqlite.dylib", "tls/libqsecuretransportbackend.dylib"):
        if not (bundle / "Contents" / "PlugIns" / plugin).exists():
            raise RuntimeError(f"The bundle carries no {plugin}, which the application cannot start without")
    ...
    # The hardened runtime refuses the memory a page is compiled into, so a bundle that turns it on without saying so opens every address as a blank one.
    signature = subprocess.run([executable("codesign"), "--display", "--entitlements", "-", "--verbose=2", str(bundle)], check=True, capture_output=True, text=True)
    described = signature.stdout + signature.stderr
    if "flags=" in described and "runtime" in described:
        for entitlement in ("com.apple.security.cs.allow-jit", "com.apple.security.cs.allow-unsigned-executable-memory", "com.apple.security.cs.disable-library-validation"):
            if entitlement not in described:
                raise RuntimeError(f"The hardened bundle declares no {entitlement}, so its web engine opens nothing")
...
def validate_installed_tree(root: Path, binary_dir: Path) -> None:
    ...
    # SQLite is the only persistence engine this product has, so a package without its driver cannot open at all.
    drivers = list(root.rglob("qsqlite.dll")) + list(root.rglob("libqsqlite.so"))
    if not drivers:
        raise RuntimeError("The package carries no SQLite driver, so the application cannot open its database")

    # Every provider, every server and every search is reached over HTTPS, which Qt answers through a backend it loads.
    backends = list(root.rglob("qschannelbackend.dll")) + list(root.rglob("qopensslbackend.dll")) + list(root.rglob("libqopensslbackend.so"))
    if not backends:
        raise RuntimeError("The package carries no TLS backend, so no provider can be reached")
