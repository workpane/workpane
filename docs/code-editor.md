...
modelling, without periodic scanning on the thread that draws. A file replaced rather than written
into is dropped by the watcher itself, so only a path it no longer holds is handed back to it, and
returning to the application looks at every open document again, because a watched file can change
without the platform reporting it at all. Every one of those changes is made once the event loop
comes back to it and only for a path that really moved, because each one restarts the stream the
engine of a platform is reading and enough of those restarts end the process.
