...
- the SQLite driver and the TLS backend, because this product has one persistence engine and reaches every provider over HTTPS
- on macOS, the assembled bundle signature and the entitlements a hardened bundle has to declare
...
That hardened runtime refuses the memory the web engine compiles a page into, so the signature that
turns it on carries the entitlements that allow it and every page would otherwise open blank. Package
validation refuses a bundle that declares the hardened runtime without them, because the difference
between a browser that works and one that opens nothing is invisible until a reader opens an address.
