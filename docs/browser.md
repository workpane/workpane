...
Each one declares the control modifier, which Qt maps to the native key of the running platform, and
answers in addition whatever else that platform names for the same action, such as `F5` for a refresh
or `Ctrl+F4` for closing. All three are rooted at the browser view, so they are inactive while another
plugin has focus. Opening a tab answers from the empty state as well, and reloading with no tab open
does nothing.
