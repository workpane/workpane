...
Each feature owns its own state, its own settings and its own translations, and reaches another one
only through a named capability rather than by calling into it.

| Feature | Identifier | Depends on | What it owns |
| --- | --- | --- | --- |
...
Each feature returns an ordered list of navigation items together with its title key, translation
catalog, style sheet, settings groups and widget factories. A navigation item declares an identifier
local to that feature, a translation key, a native icon, primary or secondary placement and the
...
belongs to the selected widget, which the core treats as opaque.

Two destinations claiming one position reject the feature that declares the second one, so the bar
never falls back to the order the libraries happened to load in. The core Settings destination is
appended after every one of them and is always last.
