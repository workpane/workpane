...
class CodeEditorTestsHelper final {
  public:
    ...
    static QSet<QRgb> highlightedColors(const QString& path, const QString& text, const CodeColorScheme& scheme);
};
...
// The numbers a server sends are decided by that server, so the client bounds them before it counts with them.
TEST(LanguageServerClientTest, AnswersTokenDeltasThatOverflowAndOnesTheProtocolDeclaresUnsigned) {
    QTemporaryDir root;
    ASSERT_TRUE(root.isValid());
    const QString path = QDir(root.path()).filePath(QStringLiteral("main.cpp"));
    LanguageServerClient client({QStringLiteral("cpp"), QCoreApplication::applicationFilePath(), {QStringLiteral("--workpane-test-lsp-hostile-numbers")}}, root.path());
    SemanticTokenSet tokens;
    bool answered = false;
    // clang-format off
    QObject::connect(&client, &LanguageServerClient::semanticTokensReady, &client, [&tokens, &answered](const QString&, const SemanticTokenSet& entries) { tokens = entries; answered = true; });
    // clang-format on
    ...
    client.requestSemanticTokens(path);
    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return answered; }));
    // clang-format on

    // Only the token the deltas really place inside the document survives, and every line kept is one the editor can paint.
    ASSERT_TRUE(tokens.contains(0));
    EXPECT_EQ(tokens.value(0).size(), 1);
    EXPECT_EQ(tokens.value(0).first().startCharacter, 4);

    for (auto entry = tokens.constBegin(); entry != tokens.constEnd(); ++entry) {
        EXPECT_GE(entry.key(), 0);
        EXPECT_LT(entry.key(), LanguageRegistry::limits().maximumSemanticTokenLines);
        for (const auto& token : entry.value()) {
            EXPECT_GE(token.startCharacter, 0);
        }
    }
...
// A character the server declares is bounded against the line before it is added to it, because the sum would overflow first.
TEST(CodeDocumentTest, LandsAtTheEndOfTheLineWhenTheServerNamesACharacterNoLineHolds) {
    filesystem::FileSystemService service;
    test::TestPluginHost host;
    host.useFileSystem(service);
    QTemporaryDir root;
    ASSERT_TRUE(root.isValid());
    const QString path = QDir(root.path()).filePath(QStringLiteral("wide.cpp"));
    ASSERT_TRUE(CodeEditorTestsHelper::writeTestFile(path, QByteArrayLiteral("int main() {}\nint second() {}\n")));
    ...
    // The second line starts past the beginning of the document, so its position added to that character is what overflows.
    document.setCursorLocation(1, std::numeric_limits<int>::max());
    EXPECT_EQ(document.cursorLine(), 2);
    EXPECT_EQ(document.cursorColumn(), QStringLiteral("int second() {}").size() + 1);

    // A line beyond the document lands on its last one rather than anywhere the sum would have put it.
    document.setCursorLocation(std::numeric_limits<int>::max(), std::numeric_limits<int>::max());
    EXPECT_EQ(document.cursorLine(), document.editor().document()->blockCount());

    // The same holds for the range a diagnostic declares, which the editor turns into the marks it paints.
    document.editor().setDiagnostics({{path, 1, std::numeric_limits<int>::max(), 1, std::numeric_limits<int>::max(), 1, QStringLiteral("wide"), QStringLiteral("W1"), QStringLiteral("fixture"), false, false, {}}});
...
// A thread leaves the register before anything can delete it, so a drain that arrives after the deferred deletions has nothing dead to quit.
TEST(LanguageServerClientTest, DrainsSafelyAfterEveryDeferredDeletionHasRun) {
    QTemporaryDir root;
    ASSERT_TRUE(root.isValid());
    const ResolvedLanguageServer server{QStringLiteral("cpp"), QDir(root.path()).filePath(QStringLiteral("missing-server")), {}};

    constexpr int transportSettleMilliseconds = 50;

    for (int cycle = 0; cycle < 40; ++cycle) {
        {
            const LanguageServerClient client(server, root.path());
        }

        QCoreApplication::processEvents(QEventLoop::AllEvents, transportSettleMilliseconds);
...
TEST(CodeEditorPluginTest, OpensAFolderOnceAndRevealsItselfWhenAnotherPluginAsks) {
    ...
    // The reply carries the structured error this plugin really has, because that error has no field a sentence could travel in and the caller owns what its own reader is told.
    EXPECT_EQ(answers.at(2).error().code, QStringLiteral("code_editor_workspace_invalid"));
    EXPECT_FALSE(answers.at(2).error().detail.isEmpty()) << "the reply names the path it refused";
...
// A write that failed is not the one the reader queued, so the queued one still reaches the file.
TEST(CodeDocumentTest, WritesTheSaveThatWasQueuedWhileTheEarlierOneWasFailing) {
    filesystem::FileSystemService service;
    test::TestPluginHost host;
    host.useFileSystem(service);
    QTemporaryDir root;
    ASSERT_TRUE(root.isValid());
    const QString path = QDir(root.path()).filePath(QStringLiteral("refused-save.cpp"));
    ASSERT_TRUE(CodeEditorTestsHelper::writeTestFile(path, QByteArrayLiteral("first\n")));

    // The first write is refused only once the event loop returns to it, which is what leaves the second save queued behind it.
    bool refused = false;
    // clang-format off
    host.writeFileHandler = [&refused, &service](const QString& target, const QByteArray& content) {
        if (refused) {
            return service.writeFile(target, content);
        }
        refused = true;
        auto promise = std::make_shared<QPromise<Result<void>>>();
        promise->start();
        const QFuture<Result<void>> future = promise->future();
        const auto reject = [promise, target]() { promise->addResult(Result<void>::failure({"test_write_refused", "The test refused this write", target})); promise->finish(); };
        QTimer::singleShot(0, QCoreApplication::instance(), reject);
        return future;
    };
    // clang-format on
    ...
    document.save();
    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return !document.dirty(); })) << failures.count();
    // clang-format on
...
// An encoding that cannot read the file is rolled back, because keeping it would fail every later reload and write the file in it.
TEST(CodeDocumentTest, RollsBackAnEncodingThatCannotReadTheFile) {
    filesystem::FileSystemService service;
    test::TestPluginHost host;
    host.useFileSystem(service);
    QTemporaryDir root;
    ASSERT_TRUE(root.isValid());
    const QString path = QDir(root.path()).filePath(QStringLiteral("latin.txt"));
    const QByteArray latin = QByteArrayLiteral("caf\xE9\n");
    ASSERT_TRUE(CodeEditorTestsHelper::writeTestFile(path, latin));
    ...
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return loaded.count() == 1; }));
    // clang-format on
    ASSERT_EQ(document.charset(), TextCharset::Latin1);
    EXPECT_EQ(document.editor().toPlainText(), QStringLiteral("café\n"));

    document.reopenWithCharset(TextCharset::Utf8);
    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return failures.count() == 1; }));
    // clang-format on
    EXPECT_EQ(document.charset(), TextCharset::Latin1);
    EXPECT_EQ(document.editor().toPlainText(), QStringLiteral("café\n"));

    // The file is still written in the encoding it was read in, rather than in the one that could not read it.
    document.editor().setPlainText(QStringLiteral("café"));
    document.save();
    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return !document.dirty(); }));
    // clang-format on
    const auto stored = test::TestFutures::awaitFuture(service.readFile(path, 1024));
    ASSERT_TRUE(stored.hasValue());
    EXPECT_EQ(stored.value(), QByteArrayLiteral("caf\xE9"));

    // A reading the file really carries still succeeds after the one it could not.
    document.reopenWithCharset(TextCharset::Latin1);
    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return loaded.count() == 2; }));
    // clang-format on
    EXPECT_EQ(failures.count(), 1);
}
...
TEST(CodeWorkspaceViewTest, SurvivesManyDocumentsOpenedEditedSavedAndClosedInOneWorkspace) {
    ...
    for (int round = 0; round < 3; ++round) {
        ...
        // A tab exists before its document has read anything, and typing into one that is still being read is the conflict the editor reports on purpose.
        // clang-format off
        const auto everyDocumentIsLoaded = [&]() { for (int index = 0; index < documents->count(); ++index) { if (qobject_cast<CodeDocument*>(documents->widget(index))->editor().toPlainText().isEmpty()) { return false; } } return true; };
        ASSERT_TRUE(test::TestFutures::waitUntil(everyDocumentIsLoaded, savingBudgetMilliseconds)) << "round " << round << " left documents unread, reported: " << failures.join(QStringLiteral(" | ")).toStdString();
        // clang-format on
...
QSet<QRgb> CodeEditorTestsHelper::highlightedColors(const QString& path, const QString& text, const CodeColorScheme& scheme) {
...
// A file replaced rather than written into is dropped by the watcher itself, so only what it no longer holds is added back.
// Removing what it still holds restarts the stream of the platform a second time, and that restart is what ends the process there.
TEST(FileWatchTest, AddsBackOnlyWhatTheWatcherNoLongerHolds) {
    QTemporaryDir directory;
    ASSERT_TRUE(directory.isValid());
    const QString kept = directory.filePath(QStringLiteral("kept.txt"));
    const QString dropped = directory.filePath(QStringLiteral("dropped.txt"));
    ASSERT_TRUE(CodeEditorTestsHelper::writeTestFile(kept, QByteArrayLiteral("one")));
    ASSERT_TRUE(CodeEditorTestsHelper::writeTestFile(dropped, QByteArrayLiteral("two")));

    QObject owner;
    QFileSystemWatcher watcher;
    ASSERT_TRUE(watcher.addPath(kept));
    ASSERT_EQ(watcher.files().size(), 1);

    // The watcher already holds this one, so nothing is handed to it and the stream it is reading is left alone.
    FileWatch::rearm(watcher, kept, &owner);
    // The watcher dropped this one the way it drops a file that was replaced, so it is the only one added back.
    FileWatch::rearm(watcher, dropped, &owner);

    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([&watcher]() { return watcher.files().size() == 2; }));
    // clang-format on
    EXPECT_TRUE(watcher.files().contains(kept));
    EXPECT_TRUE(watcher.files().contains(dropped));
}
...
TEST(CodeColorSchemeTest, PaintsOneSourceLineInManyColoursRatherThanTwo) {
    ...
    const QSet<QRgb> colors = CodeEditorTestsHelper::highlightedColors(QStringLiteral("main.cpp"), source, scheme);
...
TEST(CodeColorSchemeTest, LeavesProseAloneInsteadOfPaintingItAsCode) {
    ...
    EXPECT_TRUE(CodeEditorTestsHelper::highlightedColors(QStringLiteral("notes.txt"), prose, scheme).isEmpty()) << "a plain text file is painted as if it were code";
    ...
    const QSet<QRgb> document = CodeEditorTestsHelper::highlightedColors(QStringLiteral("README.md"), QStringLiteral("# Title\n\nIt doesn't matter, all 3 of them. See `code` and **bold**."), scheme);
