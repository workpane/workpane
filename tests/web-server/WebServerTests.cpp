...
// The same folder is opened in the editor and served at once, so a file that loses its bytes ends the connection instead of waiting for them.
TEST(WebServerInstanceTest, EndsTheConnectionWhenTheFileItIsWritingLosesItsBytes) {
    QTemporaryDir directory;
    ASSERT_TRUE(directory.isValid());
    ...
    for (int index = 0; content.size() < 4 * 1024 * 1024; ++index) {
        content.append(QByteArray::number(index)).append('\n');
    }

    const QString path = directory.filePath(QStringLiteral("asset.bin"));
    WebServerTestsHelper::writeFile(path, content);
    plugins::webserver::WebServerInstance server;
    ASSERT_TRUE(server.start(directory.path(), QStringLiteral("127.0.0.1"), 0));
    ...
    socket.write(QByteArrayLiteral("GET /asset.bin HTTP/1.1\r\nHost: localhost\r\n\r\n"));
    ASSERT_TRUE(socket.waitForBytesWritten(2000));

    QByteArray answer;
    // clang-format off
    QObject::connect(&socket, &QTcpSocket::readyRead, &socket, [&socket, &answer]() { answer.append(socket.readAll()); });
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return answer.contains(QByteArrayLiteral("\r\n\r\n")); }));
    // clang-format on
    EXPECT_TRUE(answer.startsWith(QByteArrayLiteral("HTTP/1.1 200 OK")));

    // The file loses its bytes while the transfer is still writing them, so the connection ends rather than waiting for what is gone.
    QFile shrinking(path);
    ASSERT_TRUE(shrinking.open(QIODevice::ReadWrite));
    ASSERT_TRUE(shrinking.resize(0));
    shrinking.close();

    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return socket.state() == QAbstractSocket::UnconnectedState; }, 20000)) << answer.size();
    // clang-format on
    EXPECT_LT(answer.size(), content.size());

    // Every request names the caller, whatever ended it.
    const auto logs = server.requestLog().entriesSince(0, 10);
    ASSERT_EQ(logs.entries.size(), 1);
    EXPECT_EQ(logs.entries.first().remoteAddress, QStringLiteral("127.0.0.1"));
    server.stop();
}
