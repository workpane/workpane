...
TEST(SystemInformationViewTest, RendersThemedHardwareSectionsAndRefreshState) {
    ...
    // Eighty of a hundred reads as eighty percent, because what the dependency answers is a share and never a percentage.
    QProgressBar* capacity = nullptr;

    for (auto* bar : view->findChildren<QProgressBar*>(QStringLiteral("systemInformationProgress"))) {
        const auto labels = bar->parentWidget()->findChildren<QLabel*>(QStringLiteral("systemInformationField"));
        if (!labels.isEmpty() && labels.first()->text() == QStringLiteral("Capacity")) {
            capacity = bar;
        }
    }

    ASSERT_NE(capacity, nullptr);
    EXPECT_EQ(capacity->value(), 800);
    EXPECT_EQ(capacity->maximum(), 1000);
    EXPECT_EQ(capacity->format(), QStringLiteral("80%")) << capacity->format().toStdString();
...
SystemSnapshot SystemInformationTestsHelper::sampleSnapshot() {
    ...
    snapshot.batteries = {{0, QStringLiteral("Battery Vendor"), QStringLiteral("Test Battery"), QStringLiteral("BATTERY-SERIAL"), QStringLiteral("Li-ion"), QStringLiteral("charging"), 0.8}};
