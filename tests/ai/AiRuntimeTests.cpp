...
TEST(McpClientTest, DrainsSafelyAfterEveryDeferredDeletionHasRun) {
    ...
    for (int cycle = 0; cycle < 40; ++cycle) {
        {
            const agent::mcp::McpClient client(fixture);
        }
...
TEST(AiToolRegistryTest, GivesEveryToolADeadlineExceptACommandDeclaredUnlimited) {
    ...
    // The model writes that argument, so a limit past the maximum the catalog declares carries the ordinary deadline instead of a product that cannot be held.
    const int declaredMaximum = ProviderCatalog::aiLimits().maximumCommandTimeoutSeconds;
    EXPECT_GT(registry.deadlineMsFor({QStringLiteral("c7"), QStringLiteral("run_command"), QJsonObject{{QStringLiteral("timeout_seconds"), declaredMaximum}}}), 0);
    EXPECT_EQ(registry.deadlineMsFor({QStringLiteral("c8"), QStringLiteral("run_command"), QJsonObject{{QStringLiteral("timeout_seconds"), std::numeric_limits<int>::max()}}}), ProviderCatalog::aiLimits().toolDeadlineMs);
    EXPECT_EQ(registry.deadlineMsFor({QStringLiteral("c9"), QStringLiteral("run_command"), QJsonObject{{QStringLiteral("timeout_seconds"), declaredMaximum + 1}}}), ProviderCatalog::aiLimits().toolDeadlineMs);
}

// A limit the model asks for past the one the catalog declares is refused by name, exactly as a negative one is.
TEST(AiToolRegistryTest, RefusesACommandTimeLimitPastTheOneTheCatalogDeclares) {
    test::TestPluginHost host;
    host.translations = translations::AiCatalog::english();
    filesystem::FileSystemService files;
    host.useFileSystem(files);
    ...
    QVector<ToolResult> results;
    // clang-format off
    const auto collect = [&results](const ToolResult& result) { results.append(result); };
    // clang-format on

    registry.invoke({QStringLiteral("t1"), QStringLiteral("run_command"), QJsonObject{{QStringLiteral("command"), AiTestsHelper::printWorkingDirectoryCommand()}, {QStringLiteral("timeout_seconds"), std::numeric_limits<int>::max()}}}, workdir.path(), collect);
    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return !results.isEmpty(); }));
    // clang-format on
    ASSERT_TRUE(results.first().failed);
    EXPECT_TRUE(results.first().text.contains(QStringLiteral("run_command"))) << results.first().text.toStdString();
    EXPECT_TRUE(results.first().text.contains(QStringLiteral("timeout_seconds"))) << results.first().text.toStdString();

    // The limit the catalog really declares still runs the command.
    results.clear();
    registry.invoke({QStringLiteral("t2"), QStringLiteral("run_command"), QJsonObject{{QStringLiteral("command"), AiTestsHelper::printWorkingDirectoryCommand()}, {QStringLiteral("timeout_seconds"), ProviderCatalog::aiLimits().maximumCommandTimeoutSeconds}}}, workdir.path(), collect);
    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return !results.isEmpty(); }, 20000));
    // clang-format on
...
TEST(AiTaskRepositoryTest, TakesTheDeclaredDefaultForEveryStoredValueItCannotUse) {
    ...
    // An execution value past the maximum the catalog declares is the declared default, because every reader of it counts with it.
    const QVector<QPair<QString, int>> past{{QStringLiteral("maximumIterations"), ProviderCatalog::aiLimits().maximumAgentIterations + 1}, {QStringLiteral("commandTimeoutSeconds"), ProviderCatalog::aiLimits().maximumCommandTimeoutSeconds + 1}, {QStringLiteral("parallelExecutions"), ProviderCatalog::aiLimits().maximumParallelExecutions + 1}};

    for (const auto& [key, value] : past) {
        host.settingsDocument = {{QStringLiteral("execution"), QJsonObject{{key, value}}}};
        const ExecutionSettings execution = repository.settings().execution;
        EXPECT_EQ(execution.maximumIterations, ExecutionSettings{}.maximumIterations) << key.toStdString();
        EXPECT_EQ(execution.commandTimeoutSeconds, ExecutionSettings{}.commandTimeoutSeconds) << key.toStdString();
        EXPECT_EQ(execution.parallelExecutions, ExecutionSettings{}.parallelExecutions) << key.toStdString();
    }
}

// A stored time limit no reader can count with does not load, because the shared contract holds the reader and the writer to the same range.
TEST(AiTaskRepositoryTest, RefusesAStoredCommandTimeLimitPastTheOneTheCatalogDeclares) {
    ...
    task.agentId.clear();
    task.command = QStringLiteral("true");
    task.workdir = QDir::homePath();
    task.commandTimeoutSeconds = ProviderCatalog::aiLimits().maximumCommandTimeoutSeconds;
    EXPECT_TRUE(AiTaskRepository::validTask(task));

    task.commandTimeoutSeconds = ProviderCatalog::aiLimits().maximumCommandTimeoutSeconds + 1;
    EXPECT_FALSE(AiTaskRepository::validTask(task));

    task.commandTimeoutSeconds = std::numeric_limits<int>::max();
    EXPECT_FALSE(AiTaskRepository::validTask(task));

    task.commandTimeoutSeconds = -1;
    EXPECT_FALSE(AiTaskRepository::validTask(task));
}
...
// A tool call this server never answers is what leaves a request in flight when the client is stopped.
class SilentMcpHttpServer final {
  public:
    SilentMcpHttpServer() {
        // clang-format off
        QObject::connect(&m_server, &QTcpServer::newConnection, &m_server, [this]() {
            QTcpSocket* socket = m_server.nextPendingConnection();
            auto* buffer = new QByteArray();
            QObject::connect(socket, &QTcpSocket::readyRead, socket, [this, socket, buffer]() { readRequest(socket, buffer); });
        });
        // clang-format on
    }
  ...
  private:
    void readRequest(QTcpSocket* socket, QByteArray* buffer) {
        buffer->append(socket->readAll());
        const qsizetype boundary = buffer->indexOf(QByteArrayLiteral("\r\n\r\n"));
        ...
        if (head.startsWith(QByteArrayLiteral("DELETE"))) {
            m_deleted = true;
            m_methods.append(QStringLiteral("DELETE"));
            m_sessions.append(head.toLower().contains(QByteArrayLiteral("mcp-session-id:")));
            socket->write(QByteArrayLiteral("HTTP/1.1 200 OK\r\nContent-Length: 0\r\nConnection: close\r\n\r\n"));
            socket->disconnectFromHost();
            return;
        ...
        if (method == QStringLiteral("tools/call") || !message.contains(QStringLiteral("id"))) {
            socket->write(QByteArrayLiteral("HTTP/1.1 202 Accepted\r\nContent-Length: 0\r\nConnection: close\r\n\r\n"));
            socket->disconnectFromHost();
            return;
        }
        ...
        if (method == QStringLiteral("initialize")) {
            ...
            result = QJsonObject{{QStringLiteral("tools"), QJsonArray{QJsonObject{{QStringLiteral("name"), QStringLiteral("slow_tool")}, {QStringLiteral("description"), QStringLiteral("Never answers")}, {QStringLiteral("inputSchema"), QJsonObject{{QStringLiteral("type"), QStringLiteral("object")}}}}}}};
        }

        const QByteArray body = QJsonDocument(QJsonObject{{QStringLiteral("jsonrpc"), QStringLiteral("2.0")}, {QStringLiteral("id"), message.value(QStringLiteral("id"))}, {QStringLiteral("result"), result}}).toJson(QJsonDocument::Compact);
        socket->write(QByteArrayLiteral("HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nMcp-Session-Id: session-42\r\nContent-Length: ") + QByteArray::number(body.size()) + QByteArrayLiteral("\r\nConnection: close\r\n\r\n") + body);
        socket->disconnectFromHost();
    }
...
// Stopping cancels what is still in flight at the server, so the cancellation travels before the session it names is deleted.
TEST(McpClientTest, CancelsThePendingRequestsBeforeItDeletesTheSessionTheyBelongTo) {
    SilentMcpHttpServer server;
    ASSERT_TRUE(server.listen());
    ...
    // A second start over the streamed transport is refused exactly as it is over the stdio one.
    QVector<Error> failures;
    // clang-format off
    QObject::connect(&client, &agent::mcp::McpClient::failed, &client, [&failures](const Error& error) { failures.append(error); });
    // clang-format on
    ...
    EXPECT_EQ(failures.first().code, QStringLiteral("ai_mcp_busy"));

    QVector<Result<QJsonObject>> replies;
    // clang-format off
    client.callTool(QStringLiteral("slow_tool"), {}, [&replies](Result<QJsonObject> result) { replies.append(std::move(result)); });
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return server.methods().contains(QStringLiteral("tools/call")); }));
    // clang-format on
    EXPECT_TRUE(replies.isEmpty());

    client.stop();
    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return server.deleted() && server.methods().contains(QStringLiteral("notifications/cancelled")); })) << server.methods().join(QStringLiteral(", ")).toStdString();
    // clang-format on

    const qsizetype cancelled = server.methods().indexOf(QStringLiteral("notifications/cancelled"));
    ASSERT_GE(cancelled, 0);
    // The cancellation names the session, which is the only way the server can route it to the request it ends.
    EXPECT_TRUE(server.sessions().at(cancelled)) << server.methods().join(QStringLiteral(", ")).toStdString();
    ASSERT_EQ(replies.size(), 1);
    EXPECT_FALSE(replies.first().hasValue());
}
