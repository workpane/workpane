...
// The number of seconds a service asks to be waited for is decided by that service, so it is bounded before it becomes a delay.
TEST(AiChatClientTest, BoundsARetryAfterLargeEnoughToOverflowTheDelayItAsksFor) {
    QTcpServer server;
    ASSERT_TRUE(server.listen(QHostAddress::LocalHost, 0));
    // clang-format off
    QObject::connect(&server, &QTcpServer::newConnection, &server, [&server]() {
        ...
        QObject::connect(socket, &QTcpSocket::readyRead, socket, [socket]() {
            ...
            const QByteArray body = QByteArrayLiteral("{\"error\":{\"message\":\"slow down\"}}");
    ...
    QObject::connect(&client, &AiChatClient::throttled, &client, [&waits](ThrottleReason reason, qint64 milliseconds) { if (reason == ThrottleReason::Retry) { waits.append(milliseconds); } });
    client.send({connection, QStringLiteral("http://127.0.0.1:%1").arg(server.serverPort()), QJsonArray{QJsonObject{{QStringLiteral("role"), QStringLiteral("user")}, {QStringLiteral("content"), QStringLiteral("hello")}}}, {}}, [](const QString& key) { return key; });
    ASSERT_TRUE(test::TestFutures::waitUntil([&]() { return !waits.isEmpty(); }, 10000));
    // clang-format on

    EXPECT_EQ(waits.first(), static_cast<qint64>(ProviderCatalog::aiLimits().maximumRetryBackoffMs));
    client.cancel();
}
