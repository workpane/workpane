...
TEST(TerminalWidgetTests, MovesByWordAndToTheEndsOfTheLineThroughTheSequencesAShellBinds) {
    ...
    ASSERT_EQ(backends.first()->writes.size(), 2);
    EXPECT_EQ(backends.first()->writes.at(0), QByteArrayLiteral("\033b"));
    EXPECT_EQ(backends.first()->writes.at(1), QByteArrayLiteral("\033f"));
    ...
    ASSERT_EQ(backends.first()->writes.size(), 1);
    EXPECT_NE(backends.first()->writes.at(0), QByteArrayLiteral("\033b"));
