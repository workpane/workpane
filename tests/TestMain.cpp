...
enum class LanguageServerFixture { Normal, InvalidProtocol, Crashing, Silent, SlowStart, Malformed, ManyCompletions, DeepOutline, DynamicCapabilities, TextIdentifier, HostileNumbers };
...
bool TestMainHelper::answerLanguageServerQuery(LanguageServerFixture mode, const QString& method, const QJsonObject& message) {
    ...
    if (method == QStringLiteral("textDocument/semanticTokens/full") && mode == LanguageServerFixture::HostileNumbers) {
        // One usable token, then a line delta that overflows the accumulation, then deltas the protocol declares unsigned.
        const QJsonArray hostile{0, 4, 4, 1, 0, 2147483647, 0, 4, 1, 0, 2147483647, 0, 4, 1, 0, -5, 0, 4, 1, 0, 0, -9, 4, 1, 0};
        TestMainHelper::sendLanguageServerMessage({{QStringLiteral("jsonrpc"), QStringLiteral("2.0")}, {QStringLiteral("id"), message.value(QStringLiteral("id"))}, {QStringLiteral("result"), QJsonObject{{QStringLiteral("data"), hostile}}}});
        return true;
    }
...
bool TestMainHelper::answerLanguageServerDocument(LanguageServerFixture mode, const QString& method, const QJsonObject& message) {
    ...
    if (method == QStringLiteral("textDocument/completion") && mode == LanguageServerFixture::HostileNumbers) {
        const QJsonObject wide{{QStringLiteral("start"), QJsonObject{{QStringLiteral("line"), 0}, {QStringLiteral("character"), 2147483647}}}, {QStringLiteral("end"), QJsonObject{{QStringLiteral("line"), 0}, {QStringLiteral("character"), 2147483647}}}};
        TestMainHelper::sendLanguageServerMessage({{QStringLiteral("jsonrpc"), QStringLiteral("2.0")}, {QStringLiteral("id"), message.value(QStringLiteral("id"))}, {QStringLiteral("result"), QJsonArray{QJsonObject{{QStringLiteral("label"), QStringLiteral("wide")}, {QStringLiteral("sortText"), QStringLiteral("0000")}, {QStringLiteral("textEdit"), QJsonObject{{QStringLiteral("range"), wide}, {QStringLiteral("newText"), QStringLiteral("wide")}}}}}}});
        return true;
    }
...
int main(int argc, char** argv) {
    ...
    if (TestMainHelper::hasArgument(argc, argv, "--workpane-test-lsp-hostile-numbers")) {
        return TestMainHelper::runLanguageServerFixture(LanguageServerFixture::HostileNumbers);
    }
