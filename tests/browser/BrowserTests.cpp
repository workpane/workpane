...
// A page key is native on every platform, so the control modifier is declared and whatever else the platform names for the same action joins it.
TEST(BrowserShortcutsTest, DeclaresTheNativeCombinationAndEveryBindingItsPlatformAdds) {
    const QList<QKeySequence> newTab = BrowserShortcuts::newTab();
    const QList<QKeySequence> closeTab = BrowserShortcuts::closeTab();
    const QList<QKeySequence> reload = BrowserShortcuts::reload();

    EXPECT_EQ(newTab.first(), QKeySequence(Qt::ControlModifier | Qt::Key_T));
    EXPECT_EQ(closeTab.first(), QKeySequence(Qt::ControlModifier | Qt::Key_W));
    EXPECT_EQ(reload.first(), QKeySequence(Qt::ControlModifier | Qt::Key_R));

    // clang-format off
    const auto carriesEveryBinding = [](const QList<QKeySequence>& declared, QKeySequence::StandardKey standardKey) {
        const QList<QKeySequence> bindings = QKeySequence::keyBindings(standardKey);
        return std::all_of(bindings.cbegin(), bindings.cend(), [&declared](const QKeySequence& binding) { return declared.contains(binding); });
    };
    // clang-format on

    EXPECT_TRUE(carriesEveryBinding(newTab, QKeySequence::AddTab));
    EXPECT_TRUE(carriesEveryBinding(closeTab, QKeySequence::Close));
    EXPECT_TRUE(carriesEveryBinding(reload, QKeySequence::Refresh));
}

// The keys a reader presses open a tab, close it and load the page again, and the view owns all three so no other plugin sees them.
TEST(BrowserViewTest, OpensClosesAndReloadsATabFromItsOwnKeys) {
    test::TestPluginHost host = BrowserTestsHelper::browserHost();
    BrowserPlugin plugin;
    ASSERT_TRUE(plugin.initialize(host).hasValue());
    std::unique_ptr<QWidget> view(plugin.createNavigationView(QStringLiteral("web"), nullptr));
    ...
    auto* newTab = view->findChild<QAction*>(QStringLiteral("browserNewTab"));
    auto* closeTab = view->findChild<QAction*>(QStringLiteral("browserCloseTab"));
    auto* reload = view->findChild<QAction*>(QStringLiteral("browserReload"));
    ASSERT_NE(newTab, nullptr);
    ASSERT_NE(closeTab, nullptr);
    ASSERT_NE(reload, nullptr);
    EXPECT_EQ(newTab->shortcuts(), BrowserShortcuts::newTab());
    EXPECT_EQ(closeTab->shortcuts(), BrowserShortcuts::closeTab());
    EXPECT_EQ(reload->shortcuts(), BrowserShortcuts::reload());
    EXPECT_EQ(newTab->shortcutContext(), Qt::WidgetWithChildrenShortcut);
    EXPECT_EQ(closeTab->shortcutContext(), Qt::WidgetWithChildrenShortcut);
    EXPECT_EQ(reload->shortcutContext(), Qt::WidgetWithChildrenShortcut);

    ASSERT_EQ(plugin.tabs().size(), 1);
    newTab->trigger();
    EXPECT_EQ(plugin.tabs().size(), 2);

    // Reloading asks the page that is open to load itself again, which leaves the session exactly as it was.
    const QString activeTab = plugin.tabs().at(1).id;
    reload->trigger();
    ASSERT_EQ(plugin.tabs().size(), 2);
    EXPECT_EQ(plugin.tabs().at(1).id, activeTab);

    closeTab->trigger();
    EXPECT_EQ(plugin.tabs().size(), 1);
    closeTab->trigger();
    EXPECT_TRUE(plugin.tabs().isEmpty());

    // Reloading with no tab open does nothing, and opening one answers from the empty state as well.
    reload->trigger();
    newTab->trigger();
    EXPECT_EQ(plugin.tabs().size(), 1);
