...
// A grid that repaints itself forever keeps the event loop busy, so a window of real time is what tells it apart from one that has settled.
TEST(DataGridTest, SettlesInsteadOfRepaintingItsRowsForever) {
    constexpr int settleWindowMilliseconds = 50;
    plugins::PluginManager manager;
    const ui::Theme& theme = manager.theme();
    QTableWidget* grid = ui::Components::dataGrid({QStringLiteral("Status")}, nullptr);
    grid->setRowCount(1);
    grid->setItem(0, 0, ui::Components::gridStatusItem(QStringLiteral("Running"), ui::IconName::Start, ui::ThemeColor::Success, theme));
    grid->resize(400, 120);
    grid->show();
    QCoreApplication::processEvents(QEventLoop::AllEvents, settleWindowMilliseconds);

    int changes = 0;
    // clang-format off
    QObject::connect(grid->model(), &QAbstractItemModel::dataChanged, grid, [&changes]() { ++changes; });
    // clang-format on
    QCoreApplication::processEvents(QEventLoop::AllEvents, settleWindowMilliseconds);
    EXPECT_EQ(changes, 0) << "the grid kept repainting rows nothing had changed";

    // A selection really does change the ink, so the repaint answers it and then settles again.
    grid->selectRow(0);
    // clang-format off
    ASSERT_TRUE(test::TestFutures::waitUntil([grid, &theme]() { return grid->item(0, 0)->foreground().color() == theme.color(ui::ThemeColor::OnAccent); }));
    // clang-format on

    changes = 0;
    QCoreApplication::processEvents(QEventLoop::AllEvents, settleWindowMilliseconds);
    EXPECT_EQ(changes, 0) << "the grid kept repainting the row it had already painted";
    delete grid;
}
...
TEST(MainWindowTest, OpensOnItsLoadingPageAndKeepsNothingWhenItIsClosedThere) {
    ...
    auto* caption = window.findChild<QLabel*>(QStringLiteral("startupCaption"));
    ASSERT_NE(caption, nullptr);
    EXPECT_TRUE(caption->text().endsWith(QStringLiteral("..."))) << "the caption does not say that something is still happening";
    // An indicator that was built and never started is one the reader never sees, which is what presence alone cannot tell.
    auto* indicator = window.findChild<ui::BusyIndicator*>(QStringLiteral("startupIndicator"));
    ASSERT_NE(indicator, nullptr);
    EXPECT_FALSE(indicator->isHidden()) << "the indicator was built and never started";
...
TEST(PluginManagerIntegrationTest, DiscoversInitializesAndBuildsThePluginDrivenInterface) {
...

    {
        ...
        auto* browserNewTab = window.findChild<QAction*>(QStringLiteral("browserNewTab"));
        auto* browserReload = window.findChild<QAction*>(QStringLiteral("browserReload"));
        ASSERT_NE(browserCloseTab, nullptr);
        ASSERT_NE(browserNewTab, nullptr);
        ASSERT_NE(browserReload, nullptr);

        // The control modifier is what Qt maps to the native key, so it is the combination the reader is promised.
        EXPECT_EQ(browserCloseTab->shortcuts().first(), QKeySequence(Qt::ControlModifier | Qt::Key_W));
        EXPECT_EQ(browserNewTab->shortcuts().first(), QKeySequence(Qt::ControlModifier | Qt::Key_T));
        EXPECT_EQ(browserReload->shortcuts().first(), QKeySequence(Qt::ControlModifier | Qt::Key_R));
        ...
        for (const auto& binding : QKeySequence::keyBindings(QKeySequence::Close)) {
            EXPECT_TRUE(browserCloseTab->shortcuts().contains(binding)) << binding.toString().toStdString();
        }

        EXPECT_GT(browserCloseTab->shortcuts().size(), 1);
        EXPECT_EQ(browserCloseTab->shortcutContext(), Qt::WidgetWithChildrenShortcut);
        EXPECT_EQ(browserNewTab->shortcutContext(), Qt::WidgetWithChildrenShortcut);
        EXPECT_EQ(browserReload->shortcutContext(), Qt::WidgetWithChildrenShortcut);
