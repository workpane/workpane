...
QString SystemInformationViewHelper::formatPercent(double ratio) {
    return QStringLiteral("%1%").arg(QLocale::system().toString(std::round(ratio * 100.0), 'f', 0));
}
...
void SystemInformationView::render(const SystemSnapshot& snapshot) {
    ...
    for (qsizetype index = 0; index < snapshot.batteries.size(); ++index) {
        ...
        if (battery.capacityRatio.has_value()) {
            SystemInformationViewHelper::addProgress(batteries, m_host, QStringLiteral("system-information.field.capacity"), battery.capacityRatio.value());
        }
    }
