...
Result<SystemSnapshot> HwinfoSystemInformationProvider::collect(const std::atomic_bool& cancelled) {
    ...
    for (const auto& nativeBattery : nativeBatteries) {
        ...
        // The dependency answers what is left as a share of what the battery holds full, so the range it can carry is the one every other share here carries.
        // The two it divides are a percentage on one platform and microwatt or microamp hours on another, so the share is what carries across and they do not.
        const double capacity = nativeBattery.capacity();
        if (nativeBattery.energyFull() > 0 && std::isfinite(capacity) && capacity >= 0.0 && capacity <= 1.0) {
            battery.capacityRatio = capacity;
        }
