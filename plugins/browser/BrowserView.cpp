...
BrowserView::BrowserView(BrowserPlugin& plugin, QWidget* parent) : QWidget(parent), m_plugin(plugin) {
    ...
    auto* closeTabAction = registerAction(QStringLiteral("browserCloseTab"), QStringLiteral("browser.actions.close-tab"), BrowserShortcuts::closeTab());
    auto* newTabAction = registerAction(QStringLiteral("browserNewTab"), QStringLiteral("browser.actions.new-tab"), BrowserShortcuts::newTab());
    auto* reloadAction = registerAction(QStringLiteral("browserReload"), QStringLiteral("browser.actions.reload"), BrowserShortcuts::reload());

    connect(add, &QToolButton::clicked, this, &BrowserView::createTab);
    connect(newTabAction, &QAction::triggered, this, &BrowserView::createTab);
    // clang-format off
    connect(closeTabAction, &QAction::triggered, this, [this]() { if (m_tabs->currentIndex() >= 0) { closeTab(m_tabs->currentIndex()); } });
    connect(reloadAction, &QAction::triggered, this, [this]() { if (auto* view = currentView(); view != nullptr) { view->reload(); } });
    // clang-format on
...
QAction* BrowserView::registerAction(const QString& name, const QString& translationKey, const QList<QKeySequence>& shortcuts) {
    auto* action = new QAction(m_plugin.host().translate(translationKey), this);
