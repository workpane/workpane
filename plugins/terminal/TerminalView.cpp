...
void TerminalView::createInterface() {
    ...
    // clang-format off
    connect(m_workspaceView, &WorkspaceView::interactionError, this, [this](const Error& error) { m_host.log(plugins::LogLevel::Error, QStringLiteral("terminal.interaction"), error.message, {{QStringLiteral("code"), error.code}, {QStringLiteral("detail"), error.detail}}); m_host.notify(m_host.translate(QStringLiteral("terminal.error.interaction")), TerminalFailures::terminalFailureMessage(error, m_host), plugins::AlertSeverity::Error); });
    // clang-format on
}
