...
TerminalSettings TerminalSettingsStoreHelper::settingsFromDocument(const QJsonObject& document, const QString& declaredFontFamily) {
    ...
    reader.readBool(QStringLiteral("openLinksInSystemBrowser"), settings.openLinksInSystemBrowser);
...
QJsonObject TerminalSettingsStoreHelper::settingsDocument(const TerminalSettings& settings) {
    return {{QStringLiteral("fontFamily"), settings.fontFamily}, {QStringLiteral("fontSize"), settings.fontSize}, {QStringLiteral("themeId"), settings.themeId}, {QStringLiteral("allowClipboardWrite"), settings.allowClipboardWrite}, {QStringLiteral("openLinksInSystemBrowser"), settings.openLinksInSystemBrowser}};
}
...
bool TerminalSettingsStore::openLinksInSystemBrowser() const {
    return m_settings.openLinksInSystemBrowser;
}
...
void TerminalSettingsStore::setOpenLinksInSystemBrowser(bool enabled) {
    if (enabled == m_settings.openLinksInSystemBrowser) {
        return;
    }

    auto next = m_settings;
    next.openLinksInSystemBrowser = enabled;
    apply(std::move(next));
    emit openLinksInSystemBrowserChanged(m_settings.openLinksInSystemBrowser);
}
