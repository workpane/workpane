...
WorkspaceView::WorkspaceView(plugins::terminalplugin::workspace::WorkspaceManager& manager, plugins::terminalplugin::TerminalSettingsStore& settings, plugins::PluginHost& host, QWidget* parent) : QWidget(parent), m_manager(manager), m_settings(settings), m_host(host) {
    ...
    // clang-format on
    connect(&m_settings, &plugins::terminalplugin::TerminalSettingsStore::openLinksInSystemBrowserChanged, this, &WorkspaceView::applyLinkDestination);
    synchronize();
}
...
void WorkspaceView::applyLinkDestination(bool systemBrowser) {
    for (auto* pane : m_panes) {
        pane->setOpenLinksInSystemBrowser(systemBrowser);
    }
}
...
void WorkspaceView::rebuildShelf() {
    // A chip that only left the layout is still a visible child, so it leaves its parent before the event loop collects it.
...
TerminalPane* WorkspaceView::createTerminalPane(const QString& sessionId) {
    ...
    pane->setTerminalFont(m_settings.fontFamily(), m_settings.fontSize());
    pane->setOpenLinksInSystemBrowser(m_settings.openLinksInSystemBrowser());
