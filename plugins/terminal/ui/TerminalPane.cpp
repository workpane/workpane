...
void TerminalPane::setOpenLinksInSystemBrowser(bool enabled) {
    m_openLinksInSystemBrowser = enabled;
}
...
// An address printed by a program opens where the reader asked for it, and falls back to the browser of the system when no plugin browses.
void TerminalPane::openAddress(const QString& address) {
    ...
    if (m_openLinksInSystemBrowser || !m_host.capabilityAvailable(QString::fromLatin1(plugins::openPageCapability))) {
        if (!QDesktopServices::openUrl(url)) {
            m_host.notify(m_host.translate(QStringLiteral("terminal.plugin.title")), m_host.translate(QStringLiteral("terminal.error.address-open")), plugins::AlertSeverity::Error);
        }
        return;
    }

    // clang-format off
    const auto answered = [this](Result<QJsonObject> result) { if (!result.hasValue()) { reportDestinationFailure(result.error(), QStringLiteral("terminal.error.address-open")); } };
    // clang-format on
...
void TerminalPane::offerDirectory(const QString& capability) {
    ...
    // clang-format off
    const auto answered = [this](Result<QJsonObject> result) { if (!result.hasValue()) { reportDestinationFailure(result.error(), QStringLiteral("terminal.error.destination-open")); } };
    // clang-format on
    m_host.invokeCapability(capability, {{QStringLiteral("path"), m_session->cwd()}}, *this, answered);
}

// The plugin that refused knows the condition while this one owns what the reader is told, so the diagnostic goes to the log and the sentence to the toast.
void TerminalPane::reportDestinationFailure(const Error& error, const QString& messageKey) {
    m_host.log(plugins::LogLevel::Error, QStringLiteral("terminal.destination"), error.message, {{QStringLiteral("code"), error.code}, {QStringLiteral("detail"), error.detail}});
    m_host.notify(m_host.translate(QStringLiteral("terminal.plugin.title")), m_host.translate(messageKey), plugins::AlertSeverity::Error);
}
