...
QTextCursor CodeEditorWidgetHelper::rangeCursor(const QTextDocument& document, int startLine, int startCharacter, int endLine, int endCharacter) {
    ...
    // The character is bounded against its own line before it is added to it, because the server decides it and the sum would overflow first.
    QTextCursor cursor(start);
    cursor.setPosition(start.position() + std::clamp(startCharacter, 0, start.length() - 1));
    cursor.setPosition(end.position() + std::clamp(endCharacter, 0, end.length() - 1), QTextCursor::KeepAnchor);
    return cursor;
}
