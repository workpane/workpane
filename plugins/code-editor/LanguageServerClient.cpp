...
LanguageServerClient::LanguageServerClient(ResolvedLanguageServer server, QString rootPath, QObject* parent) : QObject(parent), m_server(std::move(server)), m_rootPath(std::move(rootPath)), m_transportThread(new QThread), m_transport(new LanguageServerTransport(m_server, m_rootPath)) {
    ...
    // clang-format off
    connect(m_transportThread, &QThread::finished, m_transportThread, [thread = m_transportThread]() { const QMutexLocker locked(&LanguageServerClientHelper::transportGuard()); LanguageServerClientHelper::liveTransports().remove(thread); }, Qt::DirectConnection);
...
SemanticTokenSet LanguageServerClientHelper::decodeSemanticTokens(const QJsonArray& data, const QStringList& types, const QStringList& modifiers) {
    ...
    const qint64 lineBound = LanguageRegistry::limits().maximumSemanticTokenLines;
    SemanticTokenSet decoded;
    qint64 line = 0;
    qint64 start = 0;

    // The protocol declares every delta unsigned, so the accumulation is held in a width they cannot overflow and a token they place nowhere is dropped.
    for (qsizetype index = 0; index + 4 < data.size(); index += 5) {
        ...
        if (line < 0 || line >= lineBound || start < 0 || start > std::numeric_limits<int>::max()) {
            continue;
        }
        ...
        SemanticToken token{static_cast<int>(line), static_cast<int>(start), data.at(index + 2).toInt(), types.at(type), false, false};
        ...
        decoded[static_cast<int>(line)].append(token);
    }
