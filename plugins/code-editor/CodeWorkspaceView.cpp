...
CodeWorkspaceView::CodeWorkspaceView(CodeWorkspaceState state, QVector<ResolvedLanguageServer> languageServers, bool wordWrap, CodeEditorFont font, CodeColorScheme scheme, TextCharset defaultCharset, PluginHost& host, QWidget* parent) : QWidget(parent), m_initialState(std::move(state)), m_defaultCharset(defaultCharset), m_host(host), m_fileModel(new QFileSystemModel(this)), m_tree(new QTreeView(this)), m_documents(new ui::TabWidget(host.theme(), this)), m_problems(new QTreeWidget(this)), m_references(new QTreeWidget(this)), m_searchQuery(new QLineEdit(this)), m_searchResults(new QTreeWidget(this)), m_symbols(new QTreeWidget(this)), m_symbolSearch(new QLineEdit(this)), m_bottomPanel(new ui::TabWidget(host.theme(), this)), m_availableLanguageServers(std::move(languageServers)), m_wordWrapEnabled(wordWrap), m_font(std::move(font)), m_colorScheme(std::move(scheme)) {
    ...
    connect(&m_editorConfigWatcher, &QFileSystemWatcher::fileChanged, this, [this](const QString& changed) { FileWatch::rearm(m_editorConfigWatcher, changed, this); reloadEditorConfigs(); });
    connect(&m_editorConfigWatcher, &QFileSystemWatcher::directoryChanged, this, [this](const QString& changed) { FileWatch::rearm(m_editorConfigWatcher, changed, this); reloadEditorConfigs(); });
    // clang-format on
...
void CodeWorkspaceView::refreshEditorConfigWatch() {
    ...
    // Only what really left and what really arrived is handed over, because every other change restarts the stream of the platform for a set that did not move.
    const QStringList wanted = directories + files;
    QStringList departed;
    QStringList arrived;

    for (const auto& path : m_watchedEditorConfigPaths) {
        if (!wanted.contains(path)) {
            departed.append(path);
        }
    }

    for (const auto& path : wanted) {
        if (!m_watchedEditorConfigPaths.contains(path)) {
            arrived.append(path);
        }
    }

    m_watchedEditorConfigPaths = wanted;

    if (!departed.isEmpty()) {
        m_editorConfigWatcher.removePaths(departed);
    }

    if (!arrived.isEmpty()) {
        m_editorConfigWatcher.addPaths(arrived);
    }
}
