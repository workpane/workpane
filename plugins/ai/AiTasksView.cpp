...
void AiTasksView::requestFolderDestination(const QString& capability, const QString& path) {
    ...
    // clang-format off
    const auto answered = [this](Result<QJsonObject> result) { if (!result.hasValue()) { showError(result.error(), m_host.translate(QStringLiteral("ai.error.destination-failed"))); } };
    // clang-format on
