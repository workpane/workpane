...
// The model writes this argument, so it is refused outside the range the catalog declares rather than multiplied into a deadline it cannot hold.
bool AiToolRegistry::validCommandTimeout(const ToolCall& call) {
    const int declared = call.arguments.value(QStringLiteral("timeout_seconds")).toInt(defaultCommandTimeoutSeconds);
    return declared >= 0 && declared <= ProviderCatalog::aiLimits().maximumCommandTimeoutSeconds;
}
...
int AiToolRegistry::deadlineMsFor(const ToolCall& call) const {
    if (call.name != QStringLiteral("run_command") || !validCommandTimeout(call)) {
