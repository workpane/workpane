...
bool AiTaskRepository::validTask(const AiTask& task) {
    ...
    if (task.commandTimeoutSeconds < 0 || task.commandTimeoutSeconds > ProviderCatalog::aiLimits().maximumCommandTimeoutSeconds) {
        return false;
    }
...
QFuture<Result<void>> AiTaskRepository::saveTask(const AiTask& task) {
    if (!validTask(task) || task.position < 0 || !persistence::StoredValues::validStoredTimestamp(task.createdAtUtc) || !persistence::StoredValues::validStoredTimestamp(task.updatedAtUtc) || task.updatedAtUtc < task.createdAtUtc) {
