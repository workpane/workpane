...
qint64 AiHttpChatClient::retryDelay(const QByteArray& retryAfter) const {
    ...
    // The number of seconds is decided by whoever answered, so it is bounded before it becomes milliseconds.
    if (seconds && requested >= 0) {
        return std::min(ceiling, std::min(requested, ceiling) * 1000);
    }
