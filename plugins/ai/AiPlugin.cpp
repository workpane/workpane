...
QFuture<Result<bool>> AiPlugin::loadOlderConversation(const QString& taskId) {
    const QVector<ConversationMessage> loaded = m_conversations.value(taskId);
...
void AiPlugin::continueWhenAnswerIsPending(const QString& taskId, qint64 deliveredSequence) {
    // clang-format off
    const QVector<ConversationMessage> conversation = m_conversations.value(taskId);
...
void AiPlugin::completeToolCall(const QString& taskId, const QString& executionId, const QString& name, ToolResult result) {
    ...
    const ExecutionLogLevel level = result.failed ? ExecutionLogLevel::Warning : ExecutionLogLevel::Info;
    const QString logged = QStringLiteral("%1 %2").arg(name, result.text);
    ...
    // The entry is answered before the log announces it, because announcing reaches whoever is listening and this points inside the calls of the turn.
    appendLog(executionId, level, ExecutionLogKind::ToolReturned, logged);
