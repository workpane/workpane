...
void WebServerInstance::respond(QTcpSocket& socket, const QByteArray& request) {
    QElapsedTimer timer;
    timer.start();
    // The address is read while the socket still has one, because answering it can disconnect the socket before the entry is written.
    const QString remoteAddress = socket.peerAddress().toString();
    ...
    m_requestLog.append({0, QDateTime::currentDateTimeUtc(), QString::fromLatin1(method), QString::fromUtf8(path), status, timer.elapsed(), contentLength, remoteAddress});
}
