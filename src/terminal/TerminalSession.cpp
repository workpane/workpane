...
constexpr qsizetype maximumOutputChunkSize = 128LL * 1024;
constexpr qsizetype maximumPendingOutputSize = 1024LL * 1024;
...
Result<void> TerminalSession::sendKey(const QKeyEvent& event) {
    ...
    // A key that writes nothing is not typing, so a modifier held on its own leaves the viewport where the reader parked it.
...
// Clearing empties the screen and the history under it, and the form feed is what every shell binds to redrawing its prompt where that history was.
Result<void> TerminalSession::clearBuffer() {
    ...
    return m_backend->write(QByteArrayLiteral("\f"));
}
