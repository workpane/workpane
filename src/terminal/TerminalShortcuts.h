...
// Option moves by word and Command reaches the ends of the line, which is what every terminal of the platform answers those keys with.
constexpr auto wordModifier = Qt::AltModifier;
#else
constexpr auto terminalControlModifier = Qt::ControlModifier;
constexpr auto wordModifier = Qt::ControlModifier;
#endif
...
class TerminalShortcuts final {
  public:
    ...
    // A POSIX shell binds the meta letter while PSReadLine binds the modified arrow itself, so each platform writes the one its own shell reads.
    static QByteArray wordSequence(bool backwards) {
#ifdef Q_OS_WIN
        return backwards ? QByteArrayLiteral("\033[1;5D") : QByteArrayLiteral("\033[1;5C");
#else
        return backwards ? QByteArrayLiteral("\033b") : QByteArrayLiteral("\033f");
#endif
    }

    // A shell binds these in the keymap it opens with, so the sequence they really mean is what reaches it rather than the combination they were typed as.
    static QByteArray lineEditingSequence(const QKeyEvent& event) {
        if (event.key() != Qt::Key_Left && event.key() != Qt::Key_Right) {
            return {};
        }

        const bool backwards = event.key() == Qt::Key_Left;
        const Qt::KeyboardModifiers keyModifiers = modifiers(event);

        if (keyModifiers == wordModifier) {
            return wordSequence(backwards);
        }

#ifdef Q_OS_MACOS

        if (keyModifiers == applicationModifier) {
            return backwards ? QByteArrayLiteral("\001") : QByteArrayLiteral("\005");
        }
