...
constexpr qsizetype maximumOutputBatchSize = 64LL * 1024;
...
class PosixPtyBackendHelper final {
  public:
    ...
    static QByteArray shellArgumentZero(const QString& executable);
};
...
// A window opened by launchd inherits no profile, so the shell reads it here exactly as every terminal of that platform makes it.
// Elsewhere the desktop session already read that profile, and a login shell would cost a bash reader the file they really keep.
QByteArray PosixPtyBackendHelper::shellArgumentZero(const QString& executable) {
    const QByteArray name = QFileInfo(executable).fileName().toLocal8Bit();
#ifdef Q_OS_MACOS
    return QByteArrayLiteral("-") + name;
#else
    return name;
#endif
}
...
Result<void> PosixPtyBackend::start(const ShellProfile& profile, const QString& workingDirectory, const QString& historyFile, int columns, int rows) {
    ...
    const QByteArray argumentZero = PosixPtyBackendHelper::shellArgumentZero(profile.executable);
    ...
    arguments.push_back(const_cast<char*>(argumentZero.constData()));
    ...
    QProcessEnvironment environment = TerminalEnvironment::sessionEnvironment();
