...
constexpr qsizetype maximumInputQueueSize = 1024LL * 1024;
constexpr std::size_t outputBufferBytes = 64ULL * 1024;
...
class ConPtyBackendHelper final {
  public:
    ...
    static std::wstring environmentBlock(const QProcessEnvironment& environment);
};
...
// A Windows environment block is one run of terminated entries closed by a final terminator, which is what CreateProcessW reads.
std::wstring ConPtyBackendHelper::environmentBlock(const QProcessEnvironment& environment) {
    QStringList entries = environment.toStringList();
    entries.sort(Qt::CaseInsensitive);
    std::wstring block;

    for (const auto& entry : entries) {
        block.append(entry.toStdWString());
        block.push_back(L'\0');
    }
...
Result<void> ConPtyBackend::start(const ShellProfile& shellProfile, const QString& workingDirectory, const QString& historyFile, int columns, int rows) {
    ShellProfile profile = shellProfile;
    ...
    const auto integrationResult = WindowsShellIntegration::configure(profile, historyFile);
    ...
    std::wstring environment = ConPtyBackendHelper::environmentBlock(TerminalEnvironment::sessionEnvironment());
    const BOOL created = CreateProcessW(nullptr, commandLine.data(), nullptr, nullptr, FALSE, EXTENDED_STARTUPINFO_PRESENT | CREATE_UNICODE_ENVIRONMENT, environment.data(), cwd.c_str(), &startup.StartupInfo, &process);
    DeleteProcThreadAttributeList(attributes);
...
void ConPtyBackend::readOutput() {
    std::array<char, outputBufferBytes> buffer{};
