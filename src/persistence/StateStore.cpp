...
// Preparing, binding and running is what every statement of this store does, so the callers keep only what each of them answers with.
Result<QSqlQuery> StateStore::runStatement(const QString& statement, const QVariantList& bindings, const QString& failureCode, const QString& failureMessage) const {
    QSqlQuery sql(m_database);

    if (!sql.prepare(statement)) {
        return Result<QSqlQuery>::failure(StateStoreHelper::databaseError(QStringLiteral("database_prepare_failed"), QStringLiteral("A database statement could not be prepared"), sql.lastError()));
    }
    ...
    if (!sql.exec()) {
        return Result<QSqlQuery>::failure(StateStoreHelper::databaseError(failureCode, failureMessage, sql.lastError()));
    }

    return Result<QSqlQuery>::success(std::move(sql));
}

Result<void> StateStore::execute(const QString& statement, const QVariantList& bindings) const {
    const auto ran = runStatement(statement, bindings, QStringLiteral("database_execute_failed"), QStringLiteral("A database statement could not be executed"));
    return ran.hasValue() ? Result<void>::success() : Result<void>::failure(ran.error());
}
...
Result<void> StateStore::executeSingleRowMutation(const QString& statement, const QVariantList& bindings) const {
    ...
    if (!ran.hasValue()) {
        return Result<void>::failure(ran.error());
    }
    if (ran.value().numRowsAffected() != 1) {
...
Result<DatabaseRows> StateStore::query(const QString& statement, const QVariantList& bindings) const {
    auto ran = runStatement(statement, bindings, QStringLiteral("database_query_failed"), QStringLiteral("A database query could not be executed"));

    if (!ran.hasValue()) {
        return Result<DatabaseRows>::failure(ran.error());
    }

    QSqlQuery& sql = ran.value();
    DatabaseRows rows;
