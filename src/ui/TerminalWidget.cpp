...
void TerminalWidget::keyPressEvent(QKeyEvent* event) {
    ...
    // Moving by word and to the ends of the line is what the reader means by these keys, and a shell answers the sequence rather than the combination.
    if (const QByteArray editing = terminalcore::TerminalShortcuts::lineEditingSequence(*event); !editing.isEmpty()) {
        m_session->scrollToBottom();
        const auto written = m_session->write(editing);
        if (!written.hasValue()) {
            emit interactionError(written.error());
        }
        event->accept();
        return;
    }
...
void TerminalWidget::pasteClipboard() {
    m_session->scrollToBottom();
    const auto result = m_session->paste(QApplication::clipboard()->text().toUtf8());
...
void TerminalWidget::clearBuffer() {
    if (m_session == nullptr) {
        return;
    }

    const auto result = m_session->clearBuffer();
...
void TerminalWidget::contextMenuEvent(QContextMenuEvent* event) {
    ...
    connect(clear, &QAction::triggered, this, [this]() { clearBuffer(); });
    // clang-format on
