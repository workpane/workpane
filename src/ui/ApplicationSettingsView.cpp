...
void ApplicationSettingsView::exportConfiguration() {
    ...
    future.then(this, [this](Result<void> result) {
        m_pluginManager.notify(m_pluginManager.translate(QStringLiteral("workpane.configuration.title")), result.hasValue() ? m_pluginManager.translate(QStringLiteral("workpane.configuration.export-success")) : m_pluginManager.translate(QStringLiteral("workpane.configuration.export-error")), result.hasValue() ? plugins::AlertSeverity::Success : plugins::AlertSeverity::Error);
    });
    // clang-format on
}
