...
constexpr int iconInkRole = Qt::UserRole + 34;
...
class ComponentsHelper final {
  public:
    ...
    static void repaintGlyph(QTableWidgetItem* item, bool selected, const Theme& theme);
    static void repaintCell(QWidget* cell, bool selected, const Theme& theme);
};
...
QTableWidgetItem* Components::gridStatusItem(const QString& text, IconName name, ThemeColor role, const Theme& theme) {
    ...
    item->setData(iconInkRole, QVariant::fromValue(theme.color(role)));
...
QToolButton* Components::rowActionButton(IconName name, ThemeColor role, const Theme& theme, const QString& toolTip, QWidget* parent) {
    ...
    button->setProperty("iconInk", QVariant::fromValue(theme.color(role)));
    button->setAutoRaise(true);
    return button;
}

// A repaint writes into the rows it paints, so one that finds the ink already applied writes nothing and the grid settles instead of asking for another repaint forever.
void ComponentsHelper::repaintGlyph(QTableWidgetItem* item, bool selected, const Theme& theme) {
    if (item == nullptr || !item->data(iconNameRole).isValid()) {
        return;
    }
    ...
    if (item->data(iconInkRole).value<QColor>() == ink) {
        return;
    }

    item->setData(iconInkRole, QVariant::fromValue(ink));
    item->setIcon(IconCatalog::icon(static_cast<IconName>(item->data(iconNameRole).toInt()), ink));
    item->setForeground(ink);
}

void ComponentsHelper::repaintCell(QWidget* cell, bool selected, const Theme& theme) {
    if (cell == nullptr) {
        return;
    }
    ...
    for (auto* action : cell->findChildren<QToolButton*>()) {
        ...
        const QColor ink = selected ? theme.color(ThemeColor::OnAccent) : resting;

        if (action->property("iconInk").value<QColor>() == ink) {
            continue;
        }

        action->setProperty("iconInk", QVariant::fromValue(ink));
        action->setIcon(IconCatalog::icon(static_cast<IconName>(named.toInt()), ink));
    }
}
...
void Components::repaintRowActions(QTableWidget* grid, const Theme& theme) {
    for (int row = 0; row < grid->rowCount(); ++row) {
        ...
        for (int column = 0; column < grid->columnCount(); ++column) {
            ComponentsHelper::repaintGlyph(grid->item(row, column), selected, theme);
            ComponentsHelper::repaintCell(grid->cellWidget(row, column), selected, theme);
        }
    }
}
