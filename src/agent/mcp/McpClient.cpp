...
void McpClient::startStdio() {
    ...
    // clang-format off
    connect(transportThread, &QThread::finished, transportThread, [transportThread]() { const QMutexLocker locked(&McpClientHelper::transportGuard()); McpClientHelper::liveTransports().remove(transportThread); }, Qt::DirectConnection);
...
// The cancellations travel first, because the session they name is the one the deletion below ends.
void McpClient::stop() {
    m_startTimeout.stop();
    cancelPending();
